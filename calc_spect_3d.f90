program calc_spect
  ...
  real(8), allocatable, dimension(:,:,:) :: data2d_window, pwspt, pwspt_tot
  real(8) :: varmean
  ...
  allocate(pwspt(0:nx/2,(-ntimes/2+1):ntimes/2,(-nz/2):nz/2))
  allocate(pwspt_tot(0:nx/2,(-ntimes/2+1):ntimes/2,(-nz/2):nz/2))
    ...
    do k = 1, nz
      ...
      do t = 1, ntimes
        ...
        do i = 1, nx/2+1
           re_pw = dreal(out(i,t,k))
           im_pw = dimag(out(i,t,k))
           pwspt(i-1,t2,k2) = pwspt(i-1,t2,k2) + (re_pw**2+im_pw**2)/dble(nx)**2/dble(ntimes)**2/dble(nz)**2
        end do
      end do
    end do
  ...
  !  end do
  !end do
  pwspt_tot = pwspt_tot * 8.0d0/3.0d0
  ...
  do k = (-nz/2), nz/2
    do t = (-ntimes/2+1), ntimes/2
      do i = 0, nx/2
         if ((abs(dble(t)*2.0d0*pi/(ntimes*dt)) < 20.0d0) .and.  (abs(dble(k)*2.0d0*pi/Lz) < 41.0d0) .and. (abs(dble(i)*2.0d0*pi/Lx) < 41.0d0)) then
             write(output_file, '(4e24.16)') dble(i)*2.0d0*pi/Lx, dble(t)*2.0d0*pi/(ntimes*dt), dble(k)*2.0d0*pi/Lz, pwspt_tot(i,t,k)*Lx*Lz*ntimes*dt/(2.0d0*pi)**3
         end if
      end do
    end do
  end do
